...
set libname sgmii_pcs_share_altera_lvds_core20_191_446yfoq 
...
foreach lvds_instance_name $lvds_instance_name_list {  
   ...
   set to_pin_collection [get_keepers -nowarn ${lvds_core_instance_name}|channels[*].tx.rx_bitslip|bitslip_reset_synchronizer|sync_reg[0]] 
   if {[get_collection_size $to_pin_collection] > 0} { 
      if {$fit_flow == 1} { 
         set_max_delay -to [get_keepers -nowarn ${lvds_core_instance_name}|channels[*].tx.rx_bitslip|bitslip_reset_synchronizer|sync_reg[0]] [expr 2*$slow_clock_period_ns] 
      } else { 
         set_false_path -to [get_keepers -nowarn ${lvds_core_instance_name}|channels[*].tx.rx_bitslip|bitslip_reset_synchronizer|sync_reg[0]] 
      } 
   } 
   ...
   set to_pin_collection [get_keepers -nowarn ${lvds_core_instance_name}|channels[*].tx.rx_bitslip|bitslip_ctrl_synchronizer|sync_reg[0]] 
   if {[get_collection_size $to_pin_collection] > 0} { 
      if {$fit_flow == 1} { 
         set_max_delay -to [get_keepers -nowarn ${lvds_core_instance_name}|channels[*].tx.rx_bitslip|bitslip_ctrl_synchronizer|sync_reg[0]] [expr 2*$slow_clock_period_ns] 
      } else { 
         set_false_path -to [get_keepers -nowarn ${lvds_core_instance_name}|channels[*].tx.rx_bitslip|bitslip_ctrl_synchronizer|sync_reg[0]] 
      } 
   } 
