...
void callback(unsigned char *argument,const struct pcap_pkthdr *packet_heaher,const unsigned char *packet_content)
{
	...
	pkt = tsmp_header_switch_handle((u8*)packet_content,packet_heaher->len);
...
int main(int argc,char* argv[])
{
	...
	if(argc != 2)
	{
		printf("input format:./arp_proxy net_interface\n");
		return 0;
	}
	//libpcap initialization
	sprintf(test_rule,"%s","ether[3:1]=0x0 and ether[12:2]=0xff01");
	sprintf(temp_net_interface,"%s",argv[1]);

	arp_table_init();
	
	//intialize packet receiver and sender
	data_pkt_receive_init(test_rule,temp_net_interface);
	data_pkt_send_init(temp_net_interface);

	//receive and handle packets
	data_pkt_receive_loop(callback);
